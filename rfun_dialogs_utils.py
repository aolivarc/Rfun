...
def cut_earthquakes(data_map, arrivals, time_before, time_after, min_snr,
    ...
    for eq in arrivals["events"].keys():
        ...
        for stnm in data_map.keys():#arrivals["events"][eq]['arrivals'].keys():
            ...
            noise_var = np.var(noise.select(component="Z").filter('bandpass', freqmin=0.1, freqmax=5)[0].data)
            signal_var = np.var(stream.select(component="Z").filter('bandpass', freqmin=0.1, freqmax=5)[0].data)
